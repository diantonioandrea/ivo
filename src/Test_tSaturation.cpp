/**
 * @file Test_hSaturation.cpp
 * @author Andrea Di Antonio (github.com/diantonioandrea)
 * @brief t saturation test.
 * @date 2025-02-19
 * 
...
int main(int argc, char **argv) {
    ...
    // Output.
    std::ofstream output{"output/tSaturation_" + std::to_string(p) + "_" + std::to_string(q) + ".e21"};

    #ifndef NVERBOSE
    std::cout << "[Ivo] TEST, Testing time saturation\n" << std::endl;
    #else
    std::cout << "[Ivo] TEST, Testing time saturation" << std::endl;
    #endif
    ...
    // Tests.
    const ivo::Natural tests = 16;

    // Time elements.
    const ivo::Natural Nt = 4;

    // Space.
    const std::vector<ivo::Polygon21> space = ivo::mesher2("data/square/Square_250.p2");
    ...
    for(ivo::Natural j = 0; j < tests; ++j) {
        ...
        // Time.
        const std::vector<ivo::Real> time = ivo::mesher1(0.0L, 1.0L, Nt + j * 2);
