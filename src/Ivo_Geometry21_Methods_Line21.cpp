...
namespace ivo {
    ...
    std::optional<Point21> intersections(const Line21 &r, const Line21 &s) {
        if(distance(r, s) > constants::geometry_zero)
            return {};

        Natural rj = 0, sj = 0;
        for(Natural j = 0; j < 3; ++j) {
            if(std::abs(r(j, 0)) > constants::geometry_zero)
                for(Natural k = 0; k < 3; ++k) {
                    if(j == k)
                        continue;
                    
                    if((std::abs(s(k, 0)) > constants::geometry_zero) && (std::abs(s(k, 0) * r(j, 0) - s(j, 0) * r(k, 0)) > constants::geometry_zero)) {
                        rj = j;
                        sj = k;
                        break;
                    }
        ...
        if(std::abs(s(rj, 0)) > constants::geometry_zero)
    ...
    Real distance(const Line21 &line, const Edge21 &edge) {
        ...
        // Parallelism.
        if(std::abs(rv_2 * sv_2 - rv_sv * rv_sv) <= constants::geometry_zero)
    ...
    bool contains(const Line21 &line, const Point21 &point) {
        for(Natural j = 0; j < 3; ++j) {
            if(std::abs(line(j, 0)) > constants::geometry_zero) {
                ...
                for(Natural k = 0; k < 3; ++k) {
                    ...
                    if(std::abs(point(k) - s * line(k, 0) - line(k, 1)) > constants::geometry_zero)
                        return false;
                }
            }
        }
     ...
     * @return false 
     */
    bool spatial(const Line21 &line) { return std::abs(line(2, 0)) <= constants::geometry_zero; }
