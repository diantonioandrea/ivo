    ...
    class Vector {
        ...
        public:
            ...
            Mask operator ==(const T &scalar) const {
                ...
                std::transform(this->_entries.begin(), this->_entries.end(), comparison._entries.begin(), [scalar](const T &entry){ return std::abs(entry - scalar) <= constants::zero; });
                return comparison;
            }
            ...
            Mask operator ==(const Vector &vector) const {
                ...
                Mask comparison{this->_size};
                std::transform(this->_entries.begin(), this->_entries.end(), vector._entries.begin(), comparison._entries.begin(), [](const T &t_entry, const T &v_entry){ return std::abs(t_entry - v_entry) <= constants::zero; });
                return comparison;
            }
